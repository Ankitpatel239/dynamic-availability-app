  ...
  const TypingEffect = ({ text }: any) => {
    ...
    useEffect(() => {
      if (index < text.length) {
        ...
        // Reset after text is fully displayed
        const resetTimer = setTimeout(() => {
          setDisplayedText("");
          setIndex(0);
        }, 2000); // Adjust delay before resetting here

        return () => clearTimeout(resetTimer);
      }
    }, [index, text]);

    return <h1 className="text-2xl font-bold">{displayedText}</h1>;
  };
  return (
    <div className="flex flex-col min-h-screen">
      <main className="flex-1 bg-gray-100  flex justify-between p-6">
        <div className="relative p-6 mt-10 mt-10 text flex flex-col min-h-screen">
          {/* Fixed container for TypingEffect */}
          <div className="absolute top-0 left-0 right-0 mb-5">
            <TypingEffect text="Welcome to Availability Scheduler" />
          </div>
          <div className="pt-20">
            {" "}
            {/* Add padding-top to accommodate the fixed element */}
            <p className="mt-4 text-lg">
              This application helps you manage your availability and schedule
              events efficiently. Explore features like setting up appointments,
              checking availability, and more.
            </p>
            <ul className="mt-4 list-disc list-inside">
              <li>Feature 1: Manage appointments</li>
              <li>Feature 2: View availability</li>
              <li>Feature 3: Custom notifications</li>
            </ul>
          </div>
        </div>
